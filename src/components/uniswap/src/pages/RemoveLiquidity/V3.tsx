...
export default function RemoveLiquidityV3({
  location,
  match: {
    params: { positionId: tokenId },
  },
  position,
  removeLiquidity,
  removing
}: RemoveLiquidityProps & RouteComponentProps<{ positionId: string }>) {
  ...
  return <Remove position={position} removeLiquidity={removeLiquidity} removing={removing} tokenId={tokenId} />
}
function Remove({ position, removeLiquidity, removing, tokenId }: RemoveLiquidityProps & { tokenId: string }) {
  ...
  const burn = useCallback(async () => {
    if (!position?.liquidity || !liquidityPercentage || !tokenId || !removeLiquidity) {
      return
    }

    removeLiquidity(tokenId, BigNumber.from(liquidityPercentage.multiply(position.liquidity.toString()).quotient.toString()));

...
export function PositionPage({
...
}: { loading: boolean, position: PositionDetails | undefined }) {
  const { chainId, account, library } = useActiveWeb3React()
  const theme = useTheme()

  const parsedTokenId = positionDetails ? BigNumber.from(positionDetails.tokenId) : undefined
  ...
  const pool = useMemo(() => {
    if(!token0 || !token1 || !feeAmount || !slot0 || !liquidity) {
      return;
    }
  ...
  }, [liquidity, slot0, feeAmount, token0, token1]);
